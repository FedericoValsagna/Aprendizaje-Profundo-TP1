...
def recall_pattern_deleted(Xs: list, delete_percentage: float, hopfield: HopfieldNetwork,
                            save_images: bool = False, verbose: bool = True):
    error_percentages = []
    for i, x in enumerate(Xs):
        ...
        is_equal, diff_percentage, diff_count, total = compare_images(x, new_x)
        if verbose:
            print(f"Recalling image (borrado {delete_percentage}%) | Diferencias: {diff_count} de {total} valores ({diff_percentage:.2f}%)")
...
def save_image_deleted(xd, new_x, i, delete_percentage):
    ...
    deleted_path = os.path.join(OUTPUT_FOLDER_BORRADO, f"img{i}_borrado{delete_percentage}_incompleta.bmp")
    recalled_path = os.path.join(OUTPUT_FOLDER_BORRADO, f"img{i}_borrado{delete_percentage}_recuperada.bmp")
    numpy_array_to_bmp(xd, deleted_path)
    numpy_array_to_bmp(new_x, recalled_path)

    os.makedirs(OUTPUT_FOLDER_BORRADO_JPG, exist_ok=True)
    deleted_path_jpg = os.path.join(OUTPUT_FOLDER_BORRADO_JPG, f"img{i}_borrado{delete_percentage}_incompleta.jpg")
    recalled_path_jpg = os.path.join(OUTPUT_FOLDER_BORRADO_JPG, f"img{i}_borrado{delete_percentage}_recuperada.jpg")
...
def ej1_borrado():
    ...
    for delete_percentage in DELETE_PERCENTAGES:
        print()
        print(f"Recalling images with {delete_percentage}% borrado ({REPETITIONS} corridas)")
